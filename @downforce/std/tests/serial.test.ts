...
const dataInput = [{
    'Hello World': {
        1: [
            null, true, 1, date, regexp1, regexp2,
            {
                ...
                set: new Set<any>([
                    null, true, 1, date, regexp1, regexp2, objectLeaf,
                ]),
            },
        ],
    },
...
describe('@downforce/std/serial', (ctx) => {
    test('serializeStructSync()', async (ctx) => {
        Assert.equal(serializeStructSync(dataInput), payloadSerialized)
    })

    test('deserializeStructSync()', async (ctx) => {
        Assert.deepEqual(deserializeStructSync(payloadSerialized), dataInput)
    })

    test('serializeStructAsync()', async (ctx) => {
        Assert.equal(await serializeStructAsync(dataInput), payloadSerialized)
    })

    test('deserializeStructAsync()', async (ctx) => {
        Assert.deepEqual(await deserializeStructAsync(payloadSerialized), dataInput)
    })
})
